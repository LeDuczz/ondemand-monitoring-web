...
import type { Screen, Mission, Drone, FlightToken, ChecklistScenario, NavId } from './types';
import { DRONE_PRIMARY, DRONE_BATTERY_LOW, DRONE_HW_FAULT, DRONE_STALE_TEL, ALL_MISSIONS, CHECKLIST, REPLACEMENT_DRONES } from './mockData';
...
export default function OperatorWorkspace() {
  ...
  const displayDrone: Drone = {
    ...drone,
    state: (() => {
      if (screen === 'in-flight' || screen === 'return-to-base') return 'ACTIVE_MISSION';
      if (['gcs-connect','preflight','control-handover','ready-to-fly','preflight-failure','drone-replacement'].includes(screen)) return 'PREFLIGHT';
  ...
  function handleLanded() { setScreen('postflight'); }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'var(--font-ui)', background: 'var(--bg)' }}>
      <Sidebar role="operator" active={navId} onChange={handleNavChange} alerts={1} />

      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        {/* Mission Control transition banner */}
        {isDark && (
          <div style={{ height: 3, background: 'linear-gradient(90deg,#4f46e5,#2563eb,#06b6d4)', flexShrink: 0 }} />
        )}

        <div className={isDark ? 'dark-ws' : ''} style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
          {/* Operator overview */}
          {screen === 'operator-overview' && (
            <OperatorOverview
              onGoMissions={() => { setScreen('mission-list'); setNavId('my-missions'); }}
          ...
          {/* Operator flow */}
          {screen === 'mission-list' && (
            <MissionList missions={ALL_MISSIONS} onSelect={handleSelectMission} onScreen={goScreen} />
          )}
          ...
          {screen === 'preflight-failure' && (
            <PreflightFailure
              checklist={checklist} scenario={scenario}
        ...
        {!isDark && <div style={{ display: 'flex', alignItems: 'center', gap: 0, padding: '5px 12px', background: '#f1f3f5', borderTop: '1px solid #e5e7eb', flexShrink: 0, overflowX: 'auto' }}>
          <span style={{ fontSize: 9, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '.08em', marginRight: 5, flexShrink: 0 }}>Role:</span>
          <span style={{ flexShrink: 0, padding: '3px 8px', borderRadius: 4, fontSize: 9, fontWeight: 600, marginRight: 2, background: '#111827', color: '#fff', border: '1px solid #111827' }}>Drone Operator</span>

          <div style={{ width: 1, height: 14, background: '#d1d5db', margin: '0 8px', flexShrink: 0 }} />

          {/* Screen nav (operator-relevant) */}
          <span style={{ fontSize: 9, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '.08em', marginRight: 4, flexShrink: 0 }}>Screen:</span>
          {OPERATOR_SCREENS.map(([s, label]) => (
            <button key={s} onClick={() => goScreen(s)} style={{ flexShrink: 0, padding: '3px 8px', borderRadius: 4, fontSize: 9, fontWeight: 500, cursor: 'pointer', marginRight: 2, background: screen === s ? '#4f46e5' : 'transparent', color: screen === s ? '#fff' : '#6b7280', border: '1px solid transparent', transition: 'all .1s' }}>{label}</button>
          ))}

          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            <span style={{ fontSize: 9, fontWeight: 600, color: '#9ca3af' }}>Preflight:</span>
            {SCENARIO_OPTIONS.map(o => (
              <button key={o.id} onClick={() => setScenario(o.id)} style={{ padding: '3px 8px', borderRadius: 4, fontSize: 9, fontWeight: 500, cursor: 'pointer', background: scenario === o.id ? '#fffbeb' : 'transparent', color: scenario === o.id ? '#92400e' : '#9ca3af', border: scenario === o.id ? '1px solid #fde68a' : '1px solid transparent' }}>{o.label}</button>
            ))}
          </div>
        </div>}
      </div>
