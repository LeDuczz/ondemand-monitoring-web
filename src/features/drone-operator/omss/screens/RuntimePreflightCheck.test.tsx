...
describe('RuntimePreflightCheck', () => {
  it('keeps retrying automatically when the controller API starts late', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Waiting for flight controller preflight API/i)).toBeTruthy());
    expect((screen.getByRole('button', { name: /ok - go to drone operator/i }) as HTMLButtonElement).disabled).toBe(true);

    await waitFor(
      () => expect((screen.getByRole('button', { name: /ok - go to drone operator/i }) as HTMLButtonElement).disabled).toBe(false),
      { timeout: 6500 },
    );
  }, 7500);
  ...
  it('starts preflight automatically and enables OK only when READY', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockImplementationOnce(() => mockJson({ checkId: 'PF-1', status: 'CHECKING' }))
      .mockImplementation(() => mockJson(readyPayload));
    const onReady = vi.fn();

    render(<RuntimePreflightCheck onReady={onReady} />);

    expect(fetchMock).toHaveBeenCalledWith('http://localhost:8090/api/preflight/check', { method: 'POST' });
    expect((screen.getByRole('button', { name: /ok - go to drone operator/i }) as HTMLButtonElement).disabled).toBe(true);
    ...
    fireEvent.click(screen.getByRole('button', { name: /ok - go to drone operator/i }));
  ...
  it('keeps OK disabled on critical failure and retries with a new check', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input) => {
      ...
      if (url.endsWith('/api/preflight/check')) {
        attempt += 1;
        return mockJson({ checkId: attempt === 1 ? 'PF-2' : 'PF-3', status: 'CHECKING' });
      }
    ...
    await waitFor(() => expect(screen.getAllByText(/PX4 heartbeat not available/i).length).toBeGreaterThan(0));
    expect((screen.getByRole('button', { name: /ok - go to drone operator/i }) as HTMLButtonElement).disabled).toBe(true);

    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /retry failed checks/i }));
    });
