...
//   GET  /api/admin/ai-knowledge/analysis-logs
...
type AnalysisLogSeed = (typeof seedData.analysisLogs)[number]

const docs = createCollection(
  seedData.docs as DocSeed[],
) as unknown as DocSeed[]
const rules = createCollection(
  seedData.feasibilityRules as RuleSeed[],
) as unknown as RuleSeed[]
const analysisLogs = createCollection(
  seedData.analysisLogs as AnalysisLogSeed[],
) as unknown as AnalysisLogSeed[]
...
registerMockRoutes([
...

  {
    ...
    handler: ({ body }) => {
      ...
      if (!payload.docType)
        return fail(400, 'VALIDATION_ERROR', 'Loại tài liệu là bắt buộc.', {
          docType: 'Bắt buộc',
        })
      const newDoc = {
        id: `doc-new-${Date.now()}`,
...

  {
    ...
    handler: ({ params, body }) => {
      ...
      if (payload.severity !== undefined)
        rule.severity = payload.severity as RuleSeed['severity']
  ...
  {
    method: 'GET',
    path: '/api/admin/ai-knowledge/analysis-logs',
    handler: () =>
      ok({
        items: [...analysisLogs].sort(
          (a, b) =>
